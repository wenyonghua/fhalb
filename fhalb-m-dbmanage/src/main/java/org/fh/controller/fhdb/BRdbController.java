...
public class BRdbController extends BaseController {
	...
	public Object listAllTable() throws Exception{
		...
		pd = this.getPageData();
		Jurisdiction.putKeyToSession(Const.PROPERTIES, pd.getString("MSHOST"));
		String properties = Jurisdiction.getNowProperties();	//备份配置文件
		Object[] arrOb = DbFH.getTables(properties);			//获取本数据库的所有表名
	...
	public Object backupAll() throws Exception{
		...
		String properties = Jurisdiction.getNowProperties();				//备份配置文件
		String kackupPath;
		kackupPath = DbFH.getDbFH().backup("",properties).toString();		//调用数据库备份
		if(Tools.notEmpty(kackupPath) && !"errer".equals(kackupPath)){
			...
			pd.put("MSHOST", properties);									//微服务名称
			try {
				bRdbService.save(pd);
			} catch (Exception e) {
				errInfo = "error";
	...
	public Object backupTable() throws Exception{
		...
		String kackupPath;
		kackupPath = DbFH.getDbFH().backup(TABLENAME,properties).toString();	//调用数据库备份
		if(Tools.notEmpty(kackupPath) && !"errer".equals(kackupPath)){
			pd.put("FHDB_ID", this.get32UUID());								//主键
			pd.put("USERNAME", username);										//操作用户
			pd.put("BACKUP_TIME", DateUtil.date2Str(new Date()));				//备份时间
	...
	public Object dbRecover() throws Exception{
		...
		String returnStr = DbFH.getDbFH().recover(TABLENAME,SQLPATH,properties).toString();
	...
	public Object list(Page page) throws Exception{
		...
		String properties = Jurisdiction.getNowProperties();			//备份配置文件
		pd.put("MSHOST", properties);
		page.setPd(pd);
		List<PageData>	varList = bRdbService.list(page);				//列出Fhdb列表
		Map<String,String> DBmap = DbFH.getDBParameter(properties);
