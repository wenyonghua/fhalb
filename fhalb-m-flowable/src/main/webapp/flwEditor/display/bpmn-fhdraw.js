...
function _drawFlow(flow){
	...
	if(modelType != 'fhadmin-deployment'){
		if(arrHighLightedFlow.indexOf(polyline.element.id) != -1){
			paper.getById(polyline.element.id).attr({"stroke":"red"});
		}
	}
	...
	polylineInvisible.element.mouseover(function() {
		...
		if(arrHighLightedFlow.indexOf(polyline.element.id) != -1){
			paper.getById(polyline.element.id).attr({"stroke":"#FF8000"});
		}
	});
...
//绘画箭头（红色）
function _drawArrowHeadRed(line, connectionType)
{
	var doubleArrowWidth = 2 * ARROW_WIDTH;
