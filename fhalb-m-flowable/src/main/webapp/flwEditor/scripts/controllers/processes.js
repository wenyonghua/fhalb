...
angular.module('flowableModeler')
  .controller('ProcessesCtrl', ['$rootScope', '$scope', '$translate', '$http', '$timeout','$location', '$modal', function ($rootScope, $scope, $translate, $http, $timeout, $location, $modal) {
	  ...
	  $scope.deploymentProcess = function(process) {
	      $http({method: 'POST', url: FLOWABLE.CONFIG.contextRoot + '/fhmodel/deployment?modelId='+process.id+'&tm='+new Date().getTime(), data: $scope.model.process}).
          success(function(data) {
        	  if('success' == data.result){
        		  $scope.addAlertPromise($translate('PROCESS.ALERT.DEPLOYMENT-SUCCESS'), 'info');
        	  }else{
        		  $scope.addAlertPromise($translate('PROCESS.ALERT.DEPLOYMENT-ERRER'), 'error');
        	  }
          }).
          error(function(data, status, headers, config) {
        	  $scope.addAlertPromise($translate('PROCESS.ALERT.REQUEST-FAILURE'), 'error');
          });
	   };
...
angular.module('flowableModeler')
.controller('ImportProcessModelCtrl', ['$rootScope', '$scope', '$http', 'Upload', '$location', '$translate', function ($rootScope, $scope, $http, Upload, $location, $translate) {
      ...
      for (var i = 0; i < $files.length; i++) {
	      ...
	      $scope.model.loading = true;
              ...
              if(undefined == data.id){
            	  alert("文件无效，仅支持.bpmn和.bpmn20.xml");
              }else{
            	  $location.path("/editor/" + data.id);
                  $scope.$hide(); 
              }
